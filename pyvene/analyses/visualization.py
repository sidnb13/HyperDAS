...
def rotation_token_heatmap(
    rotate_layer, tokens, token_size, variables, intervention_size
):
            ...
            row.append(
                torch.norm(
                    W[
                        ...
                        j * intervention_size : (j + 1) * intervention_size,
                    ]
                )
            )
        mean = sum(row)
        heatmap.append([x / mean for x in row])
    return seaborn.heatmap(heatmap, xticklabels=tokens, yticklabels=variables)
